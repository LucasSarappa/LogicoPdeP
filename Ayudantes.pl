...
Aunque a veces cueste creerlo, los ayudantes de PDeP también son personas, con vidas
complicadas y tareas por resolver. Esta vez serán los alumnos quienes deberán darles una
mano codificando un programa en Prolog que los ayude a organizarse.
...
Ejemplo:
?- esDificil(robarseBorrador(190, turnoMañana)).
false.
?- esDificil(corregirTp(196,loMagiosDeTempeley,logico)).
true.
...
?- tareaAtrasada(hernan, X).
X = prepararParcial(objetos);

X = corregirTp(181, analiaAnalia, objetos);
false.
...
tienePareja(nitu).
tienePareja(alf).
2.a. Saber si un ayudante tiene problemitas: los tiene si tiene pareja, si el proyecto en el que labura
está en llamas o si cursa sistemas operativos.
?- tieneProblemitas(X).
X = nitu %cursa operativos
X = hugo %labura en un proyecto en llamas
...
2.b alHorno/1 Saber si un ayudante está al horno: lo está si todas sus tareas son difíciles, tiene
alguna tareas atrasada y tiene problemitas.
?- alHorno(X).
X = hugo %tiene todas tareas difíciles, está atrasado y tiene problemitas.

3. Y también hay que pensar
...
3.c ¿Son inversibles los predicados de los items 2.a y 2.b? ¿Por qué? En caso afirmativo, ¿Cómo se
logró que lo sean? En caso negativo, ¿Cómo modificar la solución para que lo sea?
