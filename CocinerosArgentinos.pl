...
Mariano es amigo de Susana y de Hernán Hernán es amigo de Pedro Melina es amiga de Carlos Carlos es amigo de Susana Susana no tiene amigos
...
Completar la base de conocimiento con todo lo necesario, si hay algo que no se agrega explicar por qué. Definir los siguientes predicados de manera que sean totalmente inversibles. Usar recursividad sólo si es necesario.
Verificar si una comida es saludable, se considera saludable si es:
...
noEsSaludable/1 verificar si un cocinero no es saludable, es decir de todos los platos que elabora hay uno sólo que es saludable el resto no lo son.

noUsaIngredientesPopulares/1 verificar si un cocinero no usa ingredientes populares en ninguno de sus platos.

ingredientePopularMasUsado/2 relaciona un cocinero con el ingrediente popular que más utiliza en sus comidas.

esRecomendadoPorColega/2 verificar si un cocinero puede ser recomendado por otro si es un cocinero saludable y además se cumple que:
